<div align="center">

# 韶关学院校园网自动连接系统

--------------


[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Python 3.9+](https://img.shields.io/badge/Python-3.9%2B-blue.svg)](https://python.org)
[![Stars](https://img.shields.io/github/stars/ZX-RAY1124/Shaoguan_University_netwotk_connection)](https://github.com/ZX-RAY1124/Shaoguan_University_netwotk_connection)
[![Nodejs version](https://img.shields.io/badge/Nodejs_ver.-21.16.0-yellow)](https://nodejs.org/zh-cn/download)
[![Used place](https://img.shields.io/badge/only_use_in-Shaoguan_University-brown)](https://cas.sgu.edu.cn/)

</div>   

> 本脚本由化土学院 ZX_RAYER 和信工学院 Lxy_Freely制作  
> **使用库：** `selenium` `os` `sys`  
> **Python 版本：** 3.13  
> _MIT License_

软件下载地址：   
[NodeJS版本]   
安装包：[点我下载🚀](https://github.com/ZX-RAY1124/Shaoguan_University_network_connection/releases/download/npm_version/Setup.1.0.0.exe)    
压缩包：[点我下载🚀](https://github.com/ZX-RAY1124/Shaoguan_University_network_connection/releases/download/npm_version/ver1.0.0.zip)      
# 🫵应用介绍    

是的，朋友，我知道连接校园网只需要用手点一下按钮就可以了

**但是……**

> ## 😅我真的懒得用手点😅
>> ### 😡长期坐轮椅上的迪客已经只剩格调了你知道吗😡
>>> #### 🤓不知道时间就是金钱吗🤓
>>>> ##### 😃有很深刻的教育意义不知道吗😃

**所以🤨**   

**朋友🫵**   

### 我开发了这款……🧐

## 🚀无需手指轮椅宇宙天雷勾地火陨石撞地球般超级无敌好用的校园网自动连接系统🚀

你可以叫他
<div align="center">

> __勾石__  
</div>

是的没错  
__校园网连接历史即将迎 来 大 变🤓__  
接下来  
__听 &ensp; 我 &ensp; 说__
---
# 🤗使用教程  
本软件有两个版本，一个`Python`版本一个`Nodejs`版本，如果你是什么都不止到的电脑小白，我推荐使用`Nodejs`版本，稳定高效，如果你懂得**如何正确操作电脑**，那你也可以选择功能更稳定的`Python`版本，但是UI实在是有点难看

使用教程也根据软件分为两个版本，您下载了哪个版本就点击哪个版本的教程喵(´▽`ʃ♡ƪ)  
> #### Nodejs版本：[点我跳转🚀](nodejs-ver.md)
> #### Python版本：[点我跳转🚀](python-ver.md)   
 至于为什么要写两个版本🤨  
这纯粹就是作者的馊主意，实在是闲得蛋疼🫠  

## 🤓为所有需要开机直连网的高雅人士们量身定制🤓  
### 🤗赶紧下载，就是现在🤗
# 如果您觉得好用，记得给本项目给个Star谢谢喵~本马楼真的靠这个生活的喵≧ ﹏ ≦   
![star](res/star.png)  

真的就在这里，点一下喵~    

![点赞](res/韶院点赞专用.png)

---
# 感谢喵，爱你喵！ヾ(≧ ▽ ≦)ゝ


